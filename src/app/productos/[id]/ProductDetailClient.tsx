...
function firstImageIndex(product: Product): number {
...
export default function ProductDetailClient({
  ...
  const product = fromProductDTO(productDto);
  const category = categoryDto ? fromCategoryDTO(categoryDto) : null;

  const [selectedImageIndex, setSelectedImageIndex] = useState(() =>
    firstImageIndex(product)
  );
  ...
  useEffect(() => {
    ...
    setQuantity(1);
    setSelectedImageIndex(firstImageIndex(product));
  }, [product.id]);

  const needsPurchaseOpts = productRequiresPurchaseSelection(product);
  const selectionComplete =
    !needsPurchaseOpts ||
    isCompletePurchaseSelection(product, purchaseSelection);
  const availableForLine = selectionComplete
    ? getAvailableStockForSelection(product, purchaseSelection)
    : 0;
  const hasAnyStock = productHasAnyStock(product);
  const canAddToCart =
    product.inStock && selectionComplete && availableForLine > 0;
  ...
  }, [purchaseSelection, selectionComplete, availableForLine]);
